...
int main(int argc, char** argv) {
	...
	while (ros::ok()) {
		...
		inverse_kinematics:message msgCalculatedAfterSubscription = subscriberFunction(nh, argc, argv); 
		publisherFunction(nh, msgCalculated, freq); 
...
inverse_kinematics::message messageRecieved(const arm_interface::commands msg) {
	return inverseK(msg);
}

inverse_kinematics::message subscriberFunction (ros::NodeHandle nh, int argc, char** argv) {
	//initializing the node
...
void pubisherFunction (ros::NodeHandle nh, inverse_kinematics::message msg, int freq){
	//create publisher object
	ros::Publisher pub_user_to_can = nh.advertise<inverse_kinematics::message>("CAN", 1000);
	ros::Rate rate(freq);
