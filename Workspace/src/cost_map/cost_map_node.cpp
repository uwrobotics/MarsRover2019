...
void get_object_info(); // Look into format of messages published by camera
                        // node. @AYUSH
...
void publishCostmap(cv::Mat &costmap, double resolution,
...
void fill_new_costmap(cv::Mat &newCostmap, cv::Mat &observedMask, cv::Mat &grid,
                      obstacle_detection::obstacleDataArrayConstPtr obstacles) {
  // cv::Point2f center = ()
  // fill in open space
  cv::ellipse(newCostmap, cv::Point(center_point),
  ...
  for (auto &obstacle : obstacles->obstacles) {
    ...
    // cv::ellipse(newCostmap, center, axes, angle * 180/M_PI, 0, 360,
    // cv::Scalar(P_HIT), -1);
    // cv::ellipse(observedMask, center, axes, angle * 180/M_PI, 0, 360,
    // cv::Scalar(1), -1);
    cv::Size axes(std::max(obstacle.diam_major / RESOLUTION, 1.0),
                  std::max(obstacle.diam_minor / RESOLUTION, 1.0));
    cv::ellipse(newCostmap, center, axes, obstacle.cov_angle * 180 / M_PI, 0,
                360, cv::Scalar(P_HIT), -1);
    cv::ellipse(observedMask, center, axes, obstacle.cov_angle * 180 / M_PI, 0,
                360, cv::Scalar(1), -1);
...
void mixCostMaps(cv::Mat &main_costmap, cv::Mat &new_costmap,
                 cv::Mat &newmask) {
