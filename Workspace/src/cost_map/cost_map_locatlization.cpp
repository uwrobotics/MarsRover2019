...
void translate_map(cv::Mat &cost_map, double shift_left_by,
                   double shift_down_by) {
...
void rotate_map(cv::Mat &cost_map, double angle, cv::Point2f centre_point) {
  ...
  cv::Mat rotation_matrix =
      cv::getRotationMatrix2D(centre_point, angle * 180.0 / M_PI, 1);
  cv::warpAffine(cost_map, cost_map, rotation_matrix, cost_map.size());
}

void rotate_and_translate_map(cv::Mat &cost_map, double shift_left_by,
                              double shift_down_by, double angle,
  ...
  cv::warpAffine(cost_map, cost_map, rotation_matrix, cost_map.size(),
                 cv::INTER_LINEAR, cv::BORDER_CONSTANT, cv::Scalar(0.5));
}
...
void localization_update(cv::Mat &cost_map, double delta_x, double delta_y,
  ...
  rotate_and_translate_map(cost_map, delta_x / RESOLUTION,
                           -delta_y / RESOLUTION, delta_heading, centre_point);
}
