...
def callback(data):
    ...
    clustering = DBSCAN(eps=0.65, min_samples=5).fit(np.asarray(downpcd.points))
    ...
    for label in labels:
        ...
        if obstacle.diam_major > 6.0 or (obstacle.diam_major > 3 and obstacle.diam_minor > 3.0):
            continue #bad frame

        #print obstacle.x, obstacle.z, obstacle.diameter, obstacle.diam_major, obstacle.diam_minor, obstacle.cov_angle
...
def obstacle_detection():
    ...
    sub = rospy.Subscriber("/zed/point_cloud/cloud_registered", PointCloud2, callback, queue_size=1)
