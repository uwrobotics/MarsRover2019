...
// And :
// https://github.com/stereolabs/zed-ros-wrapper/blob/master/tutorials/zed_depth_sub_tutorial/src/zed_depth_sub_tutorial.cpp
class MapReader {
public:
  MapReader(ros::NodeHandle *node) : n(node) {
    subLeftImage = n->subscribe("/zed/zed_node/rgb/image_rect_color/compressed", 1,
                                MapReader::leftImageCallback);
    //subLeftImage = n->subscribe("/zed/zed_node/rgb/image_rect_color", 1,
    //                            MapReader::leftImageCallback);
    subDepthMap = n->subscribe("/zed/zed_node/depth/depth_registered", 1,
                               MapReader::depthMapCallback);
    subCameraInfo =
        n->subscribe("/zed/left/camera_info", 1, MapReader::cameraInfoCallback);
    leftImageWidth = 2560;
    leftImageHeight = 720;
    depthMapWidth = 2560;
    depthMapHeight = 720;
    ...
    leftImage =
        new uint8_t[leftImageWidth * leftImageHeight * 3]; // 3-channel RGB
    focalLength = 1.0;
  }
  ...
  static void depthMapCallback(const sensor_msgs::Image::ConstPtr &msg) {
    // std::memcpy(depthMap, (&msg->data[0]), sizeof(float) * mapWidth *
    // mapHeight);;
  ...
  uint8_t *getLeftImage() { return leftImage; }

  float *getDepthMap() { return depthMap; }

  float getFocalLength() { return focalLength; }

  int getDepthMapWidth() { return depthMapWidth; }
  int getDepthMapHeight() { return depthMapHeight; }
  int getLeftImageWidth() { return leftImageWidth; }
  int getLeftImageHeight() { return leftImageHeight; }
...
int main(int argc, char **argv) {
  ...
  ros::Publisher obstaclePub =
      n.advertise<obstacle_detection::obstacleDataArray>("obstacles", 100);

  ros::Duration(1.0).sleep(); // Wait for data to come in
  // SpinOnce Pattern for Callbacks:
  ...
  while (ros::ok()) {
    ...
    // float* to cv::Mat conversion:
    // https://stackoverflow.com/questions/39579398/opencv-how-to-create-mat-from-uint8-t-pointer
    cv::Mat leftImage(d.getLeftImageHeight(), d.getLeftImageWidth(), CV_8UC3,
                      leftImagePointer); // 3 channel (RGB) data
    ROS_INFO("Left Image Channels: %d\n", leftImage.channels());
    cv::Mat depthMap(d.getDepthMapHeight(), d.getDepthMapWidth(), CV_32FC1,
                     depthMapPointer); // 1-channel data
    // Image to Saliency:
    ...
    cv::normalize(depthMap, depthMapNormalized, 0, 1, cv::NORM_MINMAX,
                  CV_32FC1);
    ...
    // cv::saliency::create("SPECTRAL_RESIDUAL");
    cv::Ptr<cv::saliency::StaticSaliencySpectralResidual> saliencyAlgorithm =
    ...
    // locate the centroids of islands of 1s (contours) in the binary Saliency
    // map.
    ...
    for (int i = 0; i < contours.size(); i++) {
      ...
      diameters[i] = sqrt(4 * area / 3.14159265358979323846); // equivalent
                                                              // diameter of
                                                              // circle of same
                                                              // area as contour
    }
    ...
    // vector of msgs pattern:
    // https://answers.ros.org/question/60614/how-to-publish-a-vector-of-unknown-length-of-structs-in-ros/
    obstacle_detection::obstacleDataArray dataArray;
    for (int i = 0; i < mc.size(); i++) {
      double depth = depthMap.at<double>(
          mc[i].x, mc[i].y); // get the depth of each centroid
      double xdisplacement =
          (mc[i].x - d.getDepthMapWidth() / 2) * depth / focalLength;
