...
Arm::Arm(ArmLink shoulder, ArmLink elbow, ArmLink wrist, Claw claw,
         TurnTable turnTable, RobotFrame robotFrame, QPointF startPos,
         bool isSideView, bool isDesiredArm, std::vector<double> angles,
         QGraphicsItem *parent)
    : QGraphicsItem(parent), shoulder(shoulder), elbow(elbow), wrist(wrist),
      claw(claw), turnTable(turnTable), robotFrame(robotFrame),
      startPos(startPos), isSideView(isSideView), isDesiredArm(isDesiredArm),
      mAngles(angles), mPen(new QPen()), mBrush(new QBrush()) {
...
void Arm::setAngles(std::vector<double> angles) {
  if (angles.size() >= NUM_ANGLES) {
    ...
    mAngles[ELBOW_PITCH] =
        mAngles[SHOULDER_PITCH] + angles[ELBOW_PITCH] * M_PI / 180;
    mAngles[WRIST_PITCH] =
        mAngles[ELBOW_PITCH] + angles[WRIST_PITCH] * M_PI / 180;
  }
}

std::vector<double> Arm::getAngles() const { return mAngles; }
...
QRectF Arm::boundingRect() const {
  ...
  double width = std::max(robotFrame.length,
                          robotFrame.length / 2. + shoulder.length +
                              elbow.length + wrist.length + claw.length) +
                 startPos.x() + penWidthPadding;
  double height = robotFrame.offset + turnTable.offset + shoulder.length +
                  elbow.length + wrist.length + claw.length + startPos.y() +
                  penWidthPadding;
  double top_left_x = -width / 2., top_left_y = -height / 2.;
  return QRectF(top_left_x, top_left_y, width, height);
}

void Arm::paint(QPainter *painter, const QStyleOptionGraphicsItem *option,
                QWidget *widget) {
  painter->setRenderHint(QPainter::Antialiasing);

  if (isSideView) {
    // draw in reverse order, part that needs to appear on topper must be drawn
    // later
    if (isDesiredArm) {
    ...
    QPointF shoulder_end =
        DrawLink(painter, shoulder, startPos, mAngles[SHOULDER_PITCH]);

    DrawJoint(painter, elbow, shoulder_end);
    QPointF elbow_end =
        DrawLink(painter, elbow, shoulder_end, mAngles[ELBOW_PITCH]);

    DrawJoint(painter, wrist, elbow_end);
    QPointF wrist_end =
        DrawLink(painter, wrist, elbow_end, mAngles[WRIST_PITCH]);
    ...
    DrawJoint(painter, shoulder, startPos);
    QPointF shoulder_end =
        DrawLink(painter, shoulder, startPos, mAngles[TURNTABLE_YAW]);

    DrawJoint(painter, elbow, shoulder_end);
    QPointF elbow_end =
        DrawLink(painter, elbow, shoulder_end, mAngles[TURNTABLE_YAW]);

    DrawJoint(painter, wrist, elbow_end);
    QPointF wrist_end =
        DrawLink(painter, wrist, elbow_end, mAngles[TURNTABLE_YAW]);

    DrawClaw(painter, claw, wrist_end);
  }
}

QPointF Arm::DrawLink(QPainter *painter, ArmLink armLink, QPointF startPos,
                      double angle) {
  ...
  if (isSideView) {
    ...
    // use minus because qt y axis is flipped comparing to the standard
    // cartesian system
...
void Arm::DrawTurnTable(QPainter *painter, TurnTable turnTable,
                        QPointF startPos) {
...
void Arm::DrawRobotFrame(QPainter *painter, RobotFrame robotFrame,
...
armvizwidget::armvizwidget(QWidget *parent)
    : QWidget(parent), ui(new Ui::armvizwidget) {
  ui->setupUi(this);
  ...
  std::vector<double> desired_angles = {30 / 180.0 * M_PI,
                                        45 / 180.0 * M_PI,
                                        20 / 180.0 * M_PI,
                                        10 / 180.0 * M_PI,
                                        0,
                                        0};
  std::vector<double> actual_angles = {15 / 180.0 * M_PI,
                                       30 / 180.0 * M_PI,
                                       10 / 180.0 * M_PI,
                                       5 / 180.0 * M_PI,
                                       0,
  ...
  // ROS_INFO("%f %f %f %f", sideViewArm->scenePos().x(),
  // sideViewArm->scenePos().y(), topViewArm->pos().x(), topViewArm->pos().y());
}

armvizwidget::~armvizwidget() { delete ui; }
...
void armvizwidget::actualArmPosCallback(
    std_msgs::Float64MultiArrayConstPtr armPos) {
  ...
  topviewScene->update();
  ui->armVizSideGraphicsView->fitInView(-100, -20, 200, 160,
                                        Qt::KeepAspectRatio);
  ui->armVizTopGraphicsView->fitInView(-100, -20, 200, 160,
                                       Qt::KeepAspectRatio);
}

void armvizwidget::desiredArmPosCallback(
    std_msgs::Float64MultiArrayConstPtr armPos) {
...
Arm *armvizwidget::createArm(bool isDesiredArm, bool isSideView,
                             std::vector<double> angles) {
  if (isDesiredArm) {
    ArmLink shoulder = {SHOULDER_LEN, SHOULDER_THICK, SHOULDER_JOINT_RAD,
                        Qt::black, Qt::red};
    ArmLink elbow = {ELBOW_LEN, ELBOW_THICK, ELBOW_JOINT_RAD, Qt::black,
                     Qt::red};
    ArmLink wrist = {WRIST_LEN, WRIST_THICK, WRIST_JOINT_RAD, Qt::black,
                     Qt::red};
    Claw claw = {CLAW_LEN, CLAW_WIDTH, CLAW_THICK, Qt::black};
    TurnTable turnTable = {TURNTABLE_RAD, TURNTABLE_OFFSET, Qt::black,
                           Qt::gray};
    RobotFrame robotFrame = {ROBOT_LEN, ROBOT_WIDTH, FRAME_OFFSET, Qt::black,
                             Qt::gray};

    Arm *arm = new Arm(shoulder, elbow, wrist, claw, turnTable, robotFrame,
                       QPointF(0, 0), isSideView, isDesiredArm, angles);
    return arm;
  } else {
    ArmLink shoulder = {SHOULDER_LEN, SHOULDER_THICK, SHOULDER_JOINT_RAD,
                        Qt::blue, Qt::red};
    ArmLink elbow = {ELBOW_LEN, ELBOW_THICK, ELBOW_JOINT_RAD, Qt::blue,
                     Qt::red};
    ArmLink wrist = {WRIST_LEN, WRIST_THICK, WRIST_JOINT_RAD, Qt::blue,
                     Qt::red};
    Claw claw = {CLAW_LEN, CLAW_WIDTH, CLAW_THICK, Qt::blue};
    TurnTable turnTable = {TURNTABLE_RAD, TURNTABLE_OFFSET, Qt::blue, Qt::gray};
    RobotFrame robotFrame = {ROBOT_LEN, ROBOT_WIDTH, FRAME_OFFSET, Qt::blue,
                             Qt::gray};
