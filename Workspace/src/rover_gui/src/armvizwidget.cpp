...
Arm::Arm(ArmLink shoulder, ArmLink elbow, ArmLink wrist, Claw claw,
         ...
         bool isSideView, std::vector<double> angles, QGraphicsItem *parent)
    : QGraphicsItem(parent), shoulder(shoulder), elbow(elbow), wrist(wrist),
      claw(claw), turnTable(turnTable), robotFrame(robotFrame),
      startPos(startPos), isSideView(isSideView), mAngles(angles),
      mPen(new QPen()), mBrush(new QBrush()) {}
...
armvizwidget::armvizwidget(QWidget *parent)
  ...
  std::vector<double> angles = {30 / 180.0 * M_PI,
                                45 / 180.0 * M_PI,
                                20 / 180.0 * M_PI,
                                10 / 180.0 * M_PI,
                                0,
...
bool armvizwidget::Init(ros::NodeHandle &nh) {
  avNh = &nh;
  mPoseSub =
      nh.subscribe(ARM_POSE_TOPIC, 1, &armvizwidget::ArmPosCallback, this);
}
...
void armvizwidget::ArmPosCallback(std_msgs::Float64MultiArrayConstPtr armPos) {
  ...
  ui->armVizSideGraphicsView->fitInView(-100, -20, 200, 160,
                                        Qt::KeepAspectRatio);
  ui->armVizTopGraphicsView->fitInView(-100, -100, 200, 200,
                                       Qt::KeepAspectRatio);
}
